...
/**
 * A ticking, client-side color gradient (e.g. the rainbow) that can be resolved to a
 * concrete {@link TextColor} for the current tick.
 *
 * Port note (NeoForge -> Fabric): the original subclasses {@link TextColor} so that
 * {@code getValue()} is dynamic — a single persistent instance whose color changes over
 * time as the client ticks. Vanilla's {@code TextColor} constructors are private in this
 * MC version, so subclassing it is no longer possible at all. Instead, callers resolve a
 * fresh {@link TextColor} via {@link #resolve()} each time they build the relevant
 * {@code Style}/{@code Component} (e.g. once per tooltip render) rather than holding a
 * single {@code TextColor} whose value silently changes underneath them.
 */
public class GradientColor {
    ...
    /**
     * Resolves this gradient to a concrete {@link TextColor} for the current tick.
     * Client-side only — resolves to the gradient's first color when called server-side.
     */
    public TextColor resolve() {
        if (FabricLoader.getInstance().getEnvironmentType() == EnvType.CLIENT) {
            return TextColor.fromRgb(this.gradient[(int) (PlaceboClient.getColorTicks() * this.speed % this.gradient.length)]);
        }
        return TextColor.fromRgb(this.gradient[0]);
    }
