 ...
 * <p>
 * Sent server → client alongside the registry's {@code Content} packets but before the
 * {@code End} packet (see {@link SyncManagement}'s join/reload sync flow). The client
 * stages the resolved tag map; the {@code End} payload's existing handler binds the
 * staged tags into the registry alongside the staged content.
 */
public record TagSyncPayload(Identifier id, Map<Identifier, List<Identifier>> tags) implements CustomPacketPayload {
    ...
    public static class Provider implements PayloadProvider<TagSyncPayload> {
        ...
        @Override
        public void handleClient(TagSyncPayload msg, ClientPlayNetworking.Context ctx) {
