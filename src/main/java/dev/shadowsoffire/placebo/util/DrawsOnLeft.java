 ...
 * <p>
 * Port note (NeoForge -> Fabric): {@code net.neoforged.neoforge.client.ClientHooks
 * .gatherTooltipComponents} (NeoForge's helper for turning split {@code FormattedText} lines
 * into {@code ClientTooltipComponent}s) is replaced with the plain vanilla path —
 * {@link Language#getVisualOrder(FormattedText)} to get a {@code FormattedCharSequence}, then
 * {@link ClientTooltipComponent#create(net.minecraft.util.FormattedCharSequence)} — since no
 * image/data-component tooltip parts are involved here, just plain text.
 */
public interface DrawsOnLeft {
    ...
    default void drawOnLeft(GuiGraphicsExtractor gfx, List<? extends FormattedText> list, int y) {
        ...
        int xPos = __ths().leftPos - 16 - list.stream().map(__ths().getFont()::width).max(Integer::compare).get();
        int maxWidth = 9999;
        if (xPos < 0) {
        ...
        list.forEach(text -> {
            ...
            __ths().getFont().getSplitter().splitLines(text, _maxWidth, style, (splitLine, isBlank) -> split.add(splitLine));
        });
    ...
    default void drawOnLeft(GuiGraphicsExtractor gfx, List<? extends FormattedText> list, int y, int maxWidth) {
        ...
        list.forEach(text -> {
            Style style = text instanceof Component comp ? comp.getStyle() : Style.EMPTY;
            __ths().getFont().getSplitter().splitLines(text, maxWidth, style, (splitLine, isBlank) -> split.add(splitLine));
        });

        int xPos = __ths().leftPos - 16 - split.stream().map(__ths().getFont()::width).max(Integer::compare).get();
        this.submitTooltip(gfx, split, xPos, y);
    }
    ...
    private void submitTooltip(GuiGraphicsExtractor gfx, List<FormattedText> split, int xPos, int y) {
        List<ClientTooltipComponent> lines = split.stream()
            ...
            .toList();
        gfx.tooltip(__ths().getFont(), lines, xPos, y, DefaultTooltipPositioner.INSTANCE, null);
    }
