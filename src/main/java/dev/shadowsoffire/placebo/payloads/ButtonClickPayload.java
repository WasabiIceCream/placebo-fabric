...
public record ButtonClickPayload(int button) implements CustomPacketPayload {
    ...
    public static class Provider implements PayloadProvider<ButtonClickPayload> {
        ...
        @Override
        public void handleServer(ButtonClickPayload msg, ServerPlayNetworking.Context ctx) {
            if (ctx.player().containerMenu instanceof IButtonContainer container) {
                container.onButtonClick(msg.button);
            }
        }
